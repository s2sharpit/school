  ...
  return (
    ...
    >
      <div className="flex justify-between items-center">
        <Link href="/" onClick={() => handleScroll("header")} className="flex items-center gap-2">
        ...
        <button
          ...
          className={clsx("menu-btn flex z-10 md:hidden justify-center items-center py-1", { opened: navOpen })}
        >
          <HamSvg />
        </button>
      </div>
      ...
      <div
        className={clsx(
          ...
          navOpen ? "max-md:max-h-[calc(100vh-4rem)] max-md:overflow-y-scroll max-md:border-t" : "max-md:max-h-0 max-md:opacity-0"
        )}
      >
        <ul className="text-center py-4 md:flex justify-between max-md:space-y-5">
          {navData.map((data, index) => (
            <li key={index} className="group">
              <Link href={data.path} className={clsx("capitalize flex items-center justify-center text-gray-300 hover:text-slate-100 relative", data.nested && "pointer-events-none")}>
              ...
              {data.nested && (
                <div className="max-h-0 overflow-hidden group-hover:max-h-60 opacity-0 group-hover:opacity-100 md:absolute bg-blue-950/90 group-hover:p-2 shadow-header md:w-48 rounded-md transition-all duration-500 md:-translate-x-1/4">
                  {data.nested.map((d, i) => (
                    <Link key={i} href={`${data.path}${d.path}`} className="block capitalize w-full text-center p-2 hover:bg-blue-900/70 rounded-md transition duration-300">
                      {d.title}
                    </Link>
                  ))}
                </div>
          ...
          <li className="cursor-pointer capitalize" onClick={() => handleScroll("contact")}>
            Contact
          </li>
        </ul>
      </div>
